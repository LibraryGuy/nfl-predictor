...
# 1. PAGE SETUP
st.set_page_config(page_title="NFL Sharp: Analytics & Parlay Builder", layout="wide")
...
def load_nfl_data_pro():
    try:
        ...
        weekly_polars = nfl.load_player_stats(seasons=years)
        sched_polars = nfl.load_schedules(seasons=years)
        pbp_polars = nfl.load_pbp(seasons=[2024, 2025])
        ...
        # If anything fails, return an empty DataFrame so line 72 doesn't crash
        return pd.DataFrame()
...
if data.empty or 'player_name' not in data.columns:
    st.error("⚠️ Error: The app could not connect to the NFL Data Server.")
    st.info("This is usually caused by a network timeout. Please refresh your browser or check your connection.")
    if st.button("Force Re-Sync Data"):
        st.cache_data.clear()
        st.rerun()
    st.stop() # Stops execution here so line 72 (below) is never reached
...
# Line 72: This is now safe because the guard above stops the app if 'data' is empty
...
g1, g2 = st.columns(2)
with g1:
    st.subheader(f"🏟️ Career vs. {selected_opp}")
