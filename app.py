...
def load_nfl_data_pro():
    try:
        ...
        # --- THE FIX: FLATTEN AND DEDUPLICATE ---
        ...
        # REMOVE DUPLICATE COLUMNS (Fixes the 'DataFrame' object has no attribute 'str' error)
        w_raw = w_raw.loc[:, ~w_raw.columns.duplicated()].copy()
        s_raw = s_raw.loc[:, ~s_raw.columns.duplicated()].copy()

        # --- SMART MAPPING: SEARCH FOR KEYS ---
        ...
        # Final Clean for .str and Numeric
...
if not data.empty:
    ...
    td_prob = min(max(proj_tds * 100, 5), 95) # Scale to percentage for probability feel

    # --- 8. DASHBOARD DISPLAY ---
    st.header(f"📊 {selected_player} Projections")
    
    # 4-Metric Row (Original + New TD Prob)
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Yardage Proj", f"{proj_yds:.1f} Yds")
    c2.metric("TD Likelihood", f"{td_prob:.1f}%")
    ...
    if b2.button(f"🔥 Add Anytime TD ({td_prob:.0f}%) to Parlay", use_container_width=True):
        leg = {"Player": selected_player, "Prop": "Anytime TD (ATTD)"}
        if leg not in st.session_state.parlay_legs:
            st.session_state.parlay_legs.append(leg)
            st.rerun()

    st.plotly_chart(px.line(player_subset, x='week', y=target_yds, title="Yardage History"), use_container_width=True)
