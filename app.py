...
# 1. STEP ONE: Check for login using Streamlit's new native system
if not st.user.get("is_logged_in"):
    st.title("🏈 NFL Sharp: Pro Insights")
    st.info("Please log in to access the predictor.")
    st.button("Log in with Google", on_click=st.login)
    st.stop()

# 2. STEP TWO: Check for subscription
# This now only runs IF the user is logged in
add_auth(required=True)

# 3. YOUR APP STARTS HERE
st.title("🏈 NFL Sharp: Analytics Engine")
st.write(f"Welcome back, {st.user.email}!")
