...
if not data.empty:
    with st.sidebar:
        ...
        # Safe Stadium Selection
        stadium_names = sorted(stadium_client.get_list_of_stadium_names())
        sel_stad_name = st.selectbox("Venue (Auto-Weather)", stadium_names)
    ...
    # --- FIX FOR LINE 57: Safe Stadium & Weather Check ---
    stad_data = stadium_client.get_stadium_by_name(sel_stad_name)
    
    # Handle None response from stadium_client
    if stad_data:
    ...
    if not is_indoor:
        try:
            ...
            # Note: Some versions of nfl_stadiums prefer team names or stadium keywords
            forecast = stadium_client.get_weather_forecast_for_stadium(sel_stad_name, today_str)
            if forecast and 'hourly' in forecast:
                h = forecast['hourly']
                wind_speed = sum(h.get('wind_speed_10m', [0])[:8]) / 8
                precip_prob = max(h.get('precipitation_probability', [0])[:8])
        except:
            wind_speed, precip_prob = 5, 0 # Default fallback
    ...
    if not p_df.empty:
        ...
        weather_mod = 1.0
        if not is_indoor:
            if wind_speed > 15 and p_pos != 'RB': weather_mod *= 0.90
            if precip_prob > 50 and p_pos == 'RB': weather_mod *= 1.05
        ...
        m2.metric("Market Edge (EV)", f"{ev_edge:+.1f}%", delta="VALUE" if ev_edge > 0 else "NO VALUE")
        m3.metric("Money/Ticket Gap", f"{sharp_money - public_tickets:+.0f}%")
        m4.metric("Conditions", "Perfect (Dome)" if is_indoor else f"{wind_speed:.0f} MPH")

        fig = go.Figure()
        fig.add_trace(go.Scatter(y=p_df[stat_col], name="Historical", line=dict(color='#00ff88', width=3)))
        ...
        fig.update_layout(template="plotly_dark", title="Volume & Matchup Trend")
        st.plotly_chart(fig, use_container_width=True)
else:
    st.error("Application Data Error: Check stadium_client or data source.")
