...
# --- 1. NATIVE OIDC AUTHENTICATION GATE ---
# We check st.user.is_logged_in. This is the modern 2026 standard.
if not st.user.is_logged_in:
    ...
    # CRITICAL FIX: The button MUST trigger st.login() to start the OIDC flow.
    # We do not put st.login() inside an 'if' block without st.stop() 
    # as it can cause session loops.
    st.button("Log in with Google", on_click=st.login, type="primary")
    st.stop()

# --- 2. SUBSCRIPTION PAYWALL ---
# add_auth will now use the authenticated st.user to check Stripe.
add_auth(
    required=True,
...
def load_nfl_data_pro():
    try:
        years = [2024, 2025]
        # Using nflreadpy to pull the most recent stats
...
if data.empty:
    st.warning("⚠️ Waiting for data sync...")
    st.stop()
...
vegas_line = st.sidebar.number_input("Vegas Yardage Line", value=225.5 if player_pos == 'QB' else 65.5)
...
# --- RESULTS ---
st.header(f"📊 {selected_player} Projection")
...
st.plotly_chart(px.line(player_subset, x='week', y=target, title="Yardage Trends"), use_container_width=True)
